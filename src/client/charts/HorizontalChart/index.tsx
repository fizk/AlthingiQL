...
export default class HorizontalChart extends React.Component<HorizontalChartProps, {}> {
    ...
    public render() {
        ...
        const height =
            values.length * this.dimensions.barHeight + this.dimensions.gutter;
        const x = scaleLinear()
            .domain([0, max])
            .range([
        ...
        return (
            <svg
                ...
                <g>
                    {xTicks.map((item, i) => (
                        <line
                            className="horizontal-chart__line"
                            key={`x-line-${i}`}
                            x1={x(item)}
                ...
                <g>
                    {this.props.source.map((item, i) => (
                        <rect
                            className="horizontal-chart__bar"
                            key={`rect-${item.label}-${i}`}
                            x={
                                this.dimensions.gutter +
                                this.dimensions.labelSpace
                            }
